...
public class GenericSimpleDomainStorageReadOnly extends GenericSimpleDomainStorage {
    private static final Logger LOG = Logger.getLogger(GenericSimpleDomainStorageReadOnly.class);
    ...
    private void initStorage() {
        ...
        for(int i = 0 ; i < numberOfDomains ; ++ i) {
            ...
            Enumeration<Integer> keysSet = domainEntries.get(i).keys();

            int j = 0;
            while(keysSet.hasMoreElements()) {
                storage[i][j] = keysSet.nextElement();
                ++j;
            }
        }
