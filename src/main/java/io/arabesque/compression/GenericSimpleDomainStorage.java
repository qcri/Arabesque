...
public class GenericSimpleDomainStorage extends Storage<GenericSimpleDomainStorage> {
    protected boolean countsDirty;
    protected boolean keysOrdered;
    protected ArrayList<ConcurrentHashMap<Integer, Boolean>> domainEntries;
    ...
    public GenericSimpleDomainStorage(int numberOfDomains) {
    ...
    public GenericSimpleDomainStorage() {
        numberOfDomains = -1;
        countsDirty = false;
        keysOrdered = false;
        writerSetConsumer = new WriterSetConsumer();
    ...
    protected int[] getWordIdsOfDomain(int domainId) {
        ...
        Enumeration<Integer> keysSet = domainEntries.get(domainId).keys();
        int[] keys = new int[domainEntries.get(domainId).size()];

        int i = 0;
        while(keysSet.hasMoreElements()) {
            keys[i] = keysSet.nextElement();
            ++i;
        }
    ...
    public ArrayList<ConcurrentHashMap<Integer, Boolean>> getDomainEntries() {
       return domainEntries;
    }

    public int getNumberOfEntries() {
       int numEntries = 0;
       for (ConcurrentHashMap<Integer, Boolean> domain: domainEntries)
    ...
    private void ensureCanStoreNDomains(int nDomains) {
        ...
        for (int i = 0; i < delta; ++i) {
            domainEntries.add(new ConcurrentHashMap<Integer, Boolean>());
        }
    ...
    public void addEmbedding(Embedding embedding) {
        ...
        for (int i = 0; i < numWords; ++i)
            domainEntries.get(i).put(words.getUnchecked(i), false);
     ...
     */
    @Override
    public void aggregate(GenericSimpleDomainStorage otherDomainStorage) {
        ...
        for (int i = 0; i < numberOfDomains; ++i)
            domainEntries.get(i).putAll(otherDomainStorage.domainEntries.get(i));
    ...
    public void write(DataOutput dataOutput) throws IOException {
        ...
        for (ConcurrentHashMap<Integer, Boolean> domain : domainEntries) {
            ...
            for (Map.Entry<Integer, Boolean> entry : domain.entrySet()) {
                // write wordID
                dataOutput.writeInt(entry.getKey());
                // write value
                dataOutput.writeBoolean(entry.getValue());
            }
        }
    }
    ...
    public void write(DataOutput[] outputs, boolean[] hasContent) throws IOException {
        ...
        for (ConcurrentHashMap<Integer, Boolean> domain : domainEntries) {
            ...
            for (Integer wordId : domain.keySet()) {
            ...
            for(Map.Entry<Integer, Boolean> entry : domain.entrySet()) {
                ...
                outputs[partId].writeBoolean(entry.getValue());
            }
        }
    }
    ...
    public String toStringResume() {
        ...
        sb.append("GenericSimpleDomainStorage{");
    ...
    public String toStringDebug() {
        StringBuilder sb = new StringBuilder();
        sb.append("GenericSimpleDomainStorage{numEmbeddings=");
        sb.append(numEmbeddings);
        sb.append(", enumerations=");
        sb.append(getNumberOfEnumerations());
        sb.append(",");
        ...
        for(int i = 0 ; i < domainEntries.size() ; ++i) {
            ...
            while(keys.hasMoreElements()) {
                sb.append(keys.nextElement() + " "); // print the wordId
            }
