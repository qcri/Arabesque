...
public abstract class MainGraph<
        ...
        QE extends Edge<EL>> implements Writable {
    private static final Logger LOG = Logger.getLogger(MainGraph.class);

    private static class MainGraphInstanceHolder {
        // This instance will be thread-safe initialized on-demand
        // http://tech.puredanger.com/2007/06/15/double-checked-locking/
        static MainGraph instance = Configuration.get().createGraph();
    }

    public static <G extends MainGraph> G get() {
        return (G) MainGraphInstanceHolder.instance;
    }
    ...
    public MainGraph(Path filePath)
            throws IOException {
        init(filePath);
    }

    public MainGraph(org.apache.hadoop.fs.Path hdfsPath)
            throws IOException {
        init(hdfsPath);
    }
    ...
    public void write(DataOutput dataOutput) throws IOException {
        ...
        for (Vertex entry : vertexIndexF) {
            entry.write(dataOutput);
        }

        dataOutput.writeInt(numEdges);
        for (Edge entry : edgeIndexF) {
            entry.write(dataOutput);
        }
    ...
    public void readFields(DataInput dataInput) throws IOException {
        Integer numVertices = dataInput.readInt();
        ensureCanStoreNewVertices(numVertices);

        for (int i = 0; i < numVertices; ++i) {
            QV vertex = createVertex();
            vertex.readFields(dataInput);
            addVertex(vertex);
        }

        Integer numEdges = dataInput.readInt();
        ensureCanStoreNewEdges(numEdges);
    ...
    public HashIntIntMap getVertexNeighbourhoodNN(int vertexId) {
